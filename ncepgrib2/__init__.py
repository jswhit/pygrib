        ...
        if nx is not None and ny is not None: # rectangular grid.
            ...
            if gdsinfo[2] and gdtnum == 40: # ECMWF 'reduced' global gaussian grid.
                if expand: 
                    ...
                    if hasattr(fld,'mask'):
                        fld = ma.filled(fld)
                        fld = g2clib._redtoreg(nx, lonsperlat, fld, N.empty, order=order)
                        fld = ma.masked_values(fld,fill_value)
                    else:
                        fld = g2clib._redtoreg(nx, lonsperlat, fld, N.empty,order=order)
...
#code for computing gaussian latitudes
...
def _gaulats(k):
 ...
 Output:  gaussian latitudes in degress (N to S).
    """
# The value _eps, used for convergence tests in the iterations,
